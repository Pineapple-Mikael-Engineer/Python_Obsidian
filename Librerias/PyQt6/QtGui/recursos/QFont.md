---
title: QFont — la tipografia del texto (familia, tamano, peso, estilo)
aliases:
  - QFont
  - fuente
tags:
...
# QFont — la tipografia del texto (familia, tamano, peso, estilo)

`QFont` describe una **fuente de texto**: su familia (Arial, Courier New…), su tamano en puntos, su peso (negrita) y su estilo (cursiva, subrayado). Es una **clase de valor** (no vive en el arbol de objetos ni emite senales): se construye, se configura y se aplica a un widget con `setFont`, o al [[QPainter]] con `setFont` antes de dibujar texto. Es el valor que controla como se ve cualquier texto en la GUI.
...
> `QFont` **no** hereda de `QObject` ni emite senales: es un valor que se crea por copia y se entrega a un widget o a un painter. No se subclasea ni se conecta a slots. Por eso esta nota no lleva `classDiagram` de herencia.

## Constructor y formas de crear una fuente

```python
QFont(familia: str, pointSize: int = -1)                       # familia y tamano
QFont(familia: str, pointSize: int, weight: QFont.Weight)      # tambien el peso
```

| Forma | Ejemplo | Resultado |
|-------|---------|-----------|
| Familia y tamano | `QFont("Arial", 12)` | Arial a 12 puntos |
...
| `setPointSize(tam: int)` | `None` | fija el tamano en **puntos** |
...
| `pointSize()` | `int` | el tamano actual en puntos |
...
| El texto sale mas grande o pequeno segun la pantalla | usaste pixeles cuando querias un tamano fisico estable | usa `setPointSize` (puntos), no tamanos en pixel |
