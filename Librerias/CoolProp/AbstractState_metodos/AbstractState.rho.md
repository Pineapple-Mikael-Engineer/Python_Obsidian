...
```python
state.update(CP.PT_INPUTS, 101325, 293.15)  # Agua líquida
...
```python
state.update(CP.PT_INPUTS, 101325, 373.15)  # Vapor a 100°C
...
```python
state.update(CP.PQ_INPUTS, 101325, 0.5)  # Calidad 0.5
...
```python
state.update(CP.PT_INPUTS, 2.5e7, 400)  # Agua supercrítica
...
| Valores extremos (ej: 1e-6) | Estado cerca del punto crítico | Verificar P, T dentro de rango válido |
