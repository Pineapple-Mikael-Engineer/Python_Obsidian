---
title: np.arcsin — arcoseno (inverso del seno) elemento a elemento (ufunc)
aliases:
  - arcsin
  - np.arcsin
  - arcoseno
tags:
  - numpy
  - api/funcion
  - trigonometricas

# --- Clasificación ---
lib: numpy
mod: np
tipo: funcion

# --- Comportamiento ---
retorna: ndarray | escalar
inplace: false

# --- Dependencias ---
requiere:
  - concepto_ufuncs
  - concepto_vectorizacion

draft: false
---

# np.arcsin — arcoseno (inverso del seno) elemento a elemento (ufunc)

`np.arcsin` es una **ufunc unaria**: aplica el **arcoseno** $\arcsin(x_i)$ a cada elemento, devolviendo
el ángulo **en radianes** cuyo seno es $x_i$. Es el inverso de [[np.sin]], **no mira a sus vecinos** y
**conserva el shape**. Su rasgo definitorio es el **dominio restringido**: solo acepta $x\in[-1,1]$
(el recorrido del seno); fuera de ahí no hay ángulo real y devuelve `nan` con warning. El rango de
salida es $[-\pi/2,\pi/2]$ (la rama principal). La pregunta al usarla no es "¿qué eje desaparece?"
(no desaparece ninguno) sino **"¿está mi `x` dentro de [-1, 1]?"**.

## La idea en una fórmula

Cada elemento se transforma de forma independiente; el shape se **conserva**:

$$
z_i = \arcsin(x_i) \in \left[-\tfrac{\pi}{2},\,\tfrac{\pi}{2}\right] \qquad (n_0,\dots,n_k)\ \xrightarrow{\ \arcsin\ }\ (n_0,\dots,n_k)
$$

es la rama del seno invertible en $[-\pi/2,\pi/2]$, definida solo donde $x_i\in[-1,1]$ (ahí vive la
imagen del seno). Fuera de ese intervalo el ángulo real **no existe** → `nan`:

| `x` | $\arcsin(x)$ (rad) | en grados |
|-----|--------------------|-----------|
| `-1` | `-π/2 ≈ -1.571` | `-90°` |
| `-0.5` | `-0.524` | `-30°` |
| `0` | `0.0` | `0°` |
| `0.5` | `0.524` | `30°` |
| `1` | `π/2 ≈ 1.571` | `90°` |
| `1.5` | `nan` + warning | fuera de dominio |

## Firma

```python
np.arcsin(
    x,                 # array_like: el tensor de entrada (real, en [-1, 1])
    /,
    out=None,          # ndarray | None: destino preasignado
    *,
    where=True,        # array_like[bool]: máscara de cómputo
    casting='same_kind',  # política de conversión de tipos
    order='K',         # 'K' | 'C' | 'F' | 'A': layout de memoria del resultado
    dtype=None,        # dtype: fuerza el tipo de cómputo/salida
) -> ndarray | escalar
```

## Los parámetros en detalle

### `x` — el tensor de entrada
`array_like` **real** (ndarray, lista, escalar). Cada valor debe estar en $[-1,1]$; los enteros se
promueven a float. Fuera del dominio → `nan` + `RuntimeWarning` (ver más abajo). El shape de la
salida es el de `x`.

### `out` — escribir en un buffer existente
`ndarray` preasignado con el shape de salida. Evita asignar memoria y permite operar in-place
(`np.arcsin(arr, out=arr)`). El dtype debe ser flotante (la salida lo es) y compatible bajo `casting`.

### `where` — máscara de cómputo
`array_like` booleano broadcasteable con `x`. Solo calcula el arcoseno donde es `True`; donde es
`False`, la posición conserva el valor previo de `out` (basura si no se pasó `out`). Úsalo junto con
`out` —por ejemplo, para no evaluar posiciones que sabes fuera de dominio—.

### `dtype` — tipo de cómputo y salida
Fuerza el tipo flotante de cálculo/salida (p. ej. `float32`). No produce enteros: el resultado es
siempre flotante.

### `casting` — política de conversión
`'no'`, `'equiv'`, `'safe'`, `'same_kind'` (defecto), `'unsafe'`. Controla qué conversiones se
permiten al escribir en `out` o aplicar `dtype`.

### `order` — layout de memoria
`'K'` (defecto), `'C'`, `'F'`, `'A'`. Solo afecta al almacenamiento del resultado, no a sus valores.

## El caso N-D

`np.arcsin` se aplica **elemento a elemento** sobre cualquier dimensión: no hay `axis`, no colapsa
nada, **conserva el shape**. Un tensor `(d0, d1, d2)` entra y sale como `(d0, d1, d2)`:

```python
T = np.array([[[-1.0, -0.5], [0.0, 0.5]],
              [[1.0,  0.25], [0.75, -0.25]]])   # shape (2, 2, 2), todo en [-1, 1]
np.arcsin(T).shape       # (2, 2, 2)  → shape idéntico
np.arcsin(T)
# [[[-1.5708, -0.5236], [ 0.    ,  0.5236]],
#  [[ 1.5708,  0.2527], [ 0.8481, -0.2527]]]
```

## Vectorización

`np.arcsin` reemplaza un bucle que llamaría a `math.asin` por elemento. La versión vectorizada corre
el bucle en C, sobre memoria contigua:

```python
import math
# Bucle Python (lento):
out = np.empty_like(arr)
for i in range(arr.size):
    out.flat[i] = math.asin(arr.flat[i])

# ufunc (un único bucle en C):
out = np.arcsin(arr)
```

Es el principio de [[concepto_vectorizacion]]: describes la transformación, no la iteración. Soporta
`out`/`where`/`dtype`/`casting` como toda ufunc.

## Valor de retorno

`ndarray` (o escalar de NumPy si la entrada es escalar) con el **mismo shape** que `x`, dtype
**flotante** y valores en $[-\pi/2,\pi/2]$ (radianes):

| Entrada (`x`) | dtype salida | nota |
|---------------|--------------|------|
| `float64` | `float64` | rama principal en `[-π/2, π/2]` |
| `float32` | `float32` | conserva la precisión |
| entero (`int64`...) | `float64` | se promueve a float |
| valor fuera de `[-1, 1]` | `float` con `nan` | sin ángulo real → `nan` + warning |

```python
np.arcsin(1)                  # 1.5707963267948966  (π/2)
np.rad2deg(np.arcsin(0.5))    # 30.0  → convierte a grados
np.arcsin([0, 0.5, 1]).dtype  # float64
```

## Casos de uso

### Recuperar un ángulo desde un seno conocido
```python
angulo = np.arcsin(cateto_opuesto / hipotenusa)   # radianes
np.rad2deg(angulo)                                 # a grados si hace falta
```

### Blindar el dominio antes de invertir
```python
sen = np.array([-1.0000001, 0.3, 1.0000002])   # redondeo lo sacó de [-1, 1]
np.arcsin(np.clip(sen, -1, 1))                  # sin nan: recorta primero
```

### N-D: arcoseno por elemento de un tensor
```python
T = np.array([[[-1.0, -0.5], [0.0, 0.5]],
              [[1.0,  0.25], [0.75, -0.25]]])   # (2, 2, 2), dominio válido
np.arcsin(T)
# [[[-1.5708, -0.5236], [ 0.    ,  0.5236]],
#  [[ 1.5708,  0.2527], [ 0.8481, -0.2527]]]    # mismo shape
```

## Errores comunes

| Error | Causa | Solución |
|-------|-------|----------|
| `nan` + `RuntimeWarning` | `x` fuera de `[-1, 1]` (dominio del arcoseno) | `np.clip(x, -1, 1)` antes de invertir |
| `1.0000001` da `nan` | redondeo numérico sacó `x` del dominio | recortar con [[np.clip]] a `[-1, 1]` |
| Resultado "en grados raro" | el retorno está en **radianes** | convertir con `np.rad2deg` |
| Posiciones con basura tras `where` | `where=` sin `out` inicializado | pasar siempre `out=` junto con `where=` |

## Notas relacionadas

- [[concepto_ufuncs]] — `np.arcsin` es una ufunc unaria; hereda `out`/`where`/`dtype`/`casting`
- [[concepto_vectorizacion]] — por qué sustituye al bucle por elemento
- [[np.sin]] — la función que invierte ($\sin$ restringido a $[-\pi/2,\pi/2]$)
- [[np.arccos]] · [[np.arctan]] · [[np.arctan2]] · [[np.clip]] · [[np.rad2deg]]
