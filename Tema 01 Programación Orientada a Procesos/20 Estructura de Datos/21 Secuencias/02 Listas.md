...
Una **lista** en Python es una **colección ordenada, mutable y dinámica** que puede contener elementos de diferentes tipos. Es la estructura de datos más versátil y utilizada del lenguaje.
...
## Listas como Estructuras Clásicas

Las listas soportan de forma nativa los métodos `append()` y `pop()`, lo que permite usarlas directamente como **pila** o como **cola** sin estructuras adicionales.

### 1. Pila (LIFO)

Una **pila** sigue la política **LIFO** (*Last In, First Out*): el último elemento apilado es el primero en desapilarse. Se implementa apilando con `append()` y desapilando con `pop()` sin índice (extrae el final).
...
# Desapilar (pop) - extrae el último
tope = pila.pop()
print(f"Desapilado: {tope}, Pila: {pila}")  # Desapilado: c, Pila: ['a', 'b']
```

Ambas operaciones son **O(1)**, por lo que la lista es óptima como pila.

### 2. Cola (FIFO)

Una **cola** sigue la política **FIFO** (*First In, First Out*): el primer elemento encolado es el primero en desencolarse. Encolar con `append()` y desencolar con `pop(0)` (extrae el inicio).
...
# Desencolar (dequeue) - extrae el primero
frente = cola.pop(0)
print(f"Desencolado: {frente}, Cola: {cola}")  # Desencolado: a, Cola: ['b', 'c']
```

`pop(0)` es **O(n)**: al extraer el primer elemento Python debe desplazar todos los restantes una posición hacia la izquierda. Para colas con muchas operaciones esto degrada el rendimiento.

> [!tip] Usa `collections.deque` para colas reales
> Para colas con alto volumen de operaciones se prefiere `collections.deque`, una cola doblemente enlazada con `append()`/`popleft()` en **O(1)** por ambos extremos. Pertenece al [[Modulo y Paquetes | modulo]] `collections`.
>
> ```python
> from collections import deque
> cola = deque(["a", "b", "c"])
> cola.append("d")        # encolar O(1)
> frente = cola.popleft() # desencolar O(1) -> "a"
> ```
...
6. **Considera usar tuplas** si los datos no cambiarán (más eficientes). Ver [[03 Tuplas | tuplas]].
