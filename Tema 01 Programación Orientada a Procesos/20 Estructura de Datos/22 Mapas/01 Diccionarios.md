...
Colección **mutable y dinámica** de pares clave-valor. Desde Python 3.7 mantienen el **orden de inserción**. Implementados internamente como tablas hash, ofrecen acceso O(1) promedio.
...
fusion = dic1 | dic2
print(f"Fusión dic1 | dic2: {fusion}")  # {'a': 1, 'b': 20, 'c': 3} (derecha prevalece)

# Los originales no se modifican
print(f"dic1 intacto: {dic1}")  # {'a': 1, 'b': 2}

# Operador |= (fusión in-place, equivale a update)
dic1 |= dic2
print(f"dic1 después de |=: {dic1}")  # {'a': 1, 'b': 20, 'c': 3}
```

## Tipos Especializados de Diccionarios

Las variantes de `collections` se tratan en [[02 Diccionarios Especializados|Diccionarios Especializados]].
