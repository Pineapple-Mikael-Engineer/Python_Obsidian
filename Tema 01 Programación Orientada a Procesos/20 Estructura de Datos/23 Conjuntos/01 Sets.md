...
Un **conjunto (`set`)** es una **colección mutable, no ordenada y sin duplicados** basada en una tabla hash. Almacena únicamente elementos **hashables** y ofrece pertenencia (`in`) en **O(1) promedio**. La variante inmutable se trata en [[02 Frozenset|Frozenset]].

```python
conjunto = {1, 2, 3, 2, 1}          # los duplicados se eliminan
3 in conjunto                       # O(1) promedio
```
...
| No ordenado       | No mantiene orden de inserción de forma garantizada                                 |
| Mutable (`set`)   | Se añaden/eliminan elementos; `frozenset` no                                        |
| Hashables         | Los elementos deben ser inmutables (`int`, `str`, `tuple`, `frozenset`…)            |
| Iterable          | Recorrible con `for`                                                                |
| Eficiente         | Búsqueda, inserción y borrado en O(1) promedio                                       |
...
> [!note] Orden de inserción
> Python 3.7+ puede conservar el orden de inserción como **detalle de implementación**, pero no se debe confiar en ello: un `set` es conceptualmente no ordenado.
...
> [!warning] Conjunto vacío
> `{}` crea un **diccionario** vacío, no un set. El conjunto vacío se crea siempre con `set()`.
...
| Diferencia simétrica  | `^`      | `symmetric_difference()`     | Elementos en A o B, no en ambos    |
| Subconjunto           | `<=`     | `issubset()`                 | A ⊆ B                              |
| Superconjunto         | `>=`     | `issuperset()`               | A ⊇ B                              |
| Disjuntos             | —        | `isdisjoint()`               | Sin elementos comunes              |

> [!tip] Operador vs. método
> El operador (`|`, `&`, `-`, `^`) exige que ambos operandos sean conjuntos. El **método** acepta cualquier iterable (`A.union([5, 6])`).
...
> `remove()` lanza `KeyError` si el elemento no existe; `discard()` no falla. Usa `discard()` cuando la ausencia es aceptable.

### Modificación con operaciones
...
La variante inmutable se trata en [[02 Frozenset|Frozenset]].
...
### Detección de anomalías y filtrado
